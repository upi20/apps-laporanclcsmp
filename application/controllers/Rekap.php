...
class Rekap extends Render_Controller
{
    ...
    public function index()
    {
        if ($this->session->userdata("data")['level'] == "Admin Sekolah") {
            ...
            $this->load->view('rekap/cabang/index', $data);
        }
    }
    ...
    public function ajax_data()
    {
        ...
        $data   = $this->rekap->getAllData($length, $start, $_cari, $filter_data_kode, $filter_data_tanggal, $filter_data_cabang)->result_array();
        $count   = count($data);


        array($cari);
        $this->output_json(array('recordsTotal' => $count, 'recordsFiltered' => $count, 'draw' => $draw, 'search' => $_cari, 'data' => $data, 'mod' => $filter_data_cabang));
    }

    public function excel()
    {
        // footer laporan
        $laporan = $this->rekap->getDataLaporan();
        $filter_data_cabang = $this->input->get('cabang');
        $tanggal_start =  $this->input->get('start');
        $tanggal_end =  $this->input->get('end');
        ...
        // data body
        $detail = $this->rekap->getAllDataCetak(null, null, null, $filter_data_kode, $filter_data_tanggal, $filter_data_cabang)->result_array();
